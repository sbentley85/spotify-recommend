...
document.body.onload = function() {
    ...
    if (!sessionStorage.getItem('searchTerm')) {
        ...
        (document.getElementById('input') as HTMLInputElement).value = sessionStorage.getItem('searchTerm')!
    }
}
...
const Spotify = {
    getAccessToken () {
        ...
        if (accessTokenMatch && expiresInMatch) {
           ...
           return accessToken;
            ...
            const scopes = 'playlist-modify-public user-library-read user-top-read'
            const accessUri = `https://accounts.spotify.com/authorize?client_id=${clientId}&response_type=token&scope=${encodeURIComponent(scopes)}&redirect_uri=${redirectUri}`
    ...
    getTopArtists () {
        ...
        return fetch('https://api.spotify.com/v1/me/top/artists', {headers: headers}).then(response => response.json()
        ).then((jsonResponse) => {
    ...
    getPlaylists () {
        ...
        return fetch('https://api.spotify.com/v1/me/playlists?limit=50', {headers: headers}
            ...
            return jsonResponse.items.map((playlist: { id: number; name: string; }) => ({
                id: playlist.id,
                name: playlist.name,
            }))
        })
